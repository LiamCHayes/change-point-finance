## Identify regions where change occurred
...
pval_individual <- c(xlb=getPVals(f_t, A=1), xle=getPVals(f_t, A=2), xlf=getPVals(f_t, A=3),
                     xli=getPVals(f_t, A=4), xlk=getPVals(f_t, A=5), xlp=getPVals(f_t, A=6),
                     xlu=getPVals(f_t, A=7), xlv=getPVals(f_t, A=8), xly=getPVals(f_t, A=9))
